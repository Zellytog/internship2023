...
⟨⟩ᶠ : (A : AbGrp ℓ) → (f : FinSetₐ ℓ) → ((x : ⟨ fst f ⟩) → A →ₐ snd f x) → A →ₐ ⨁ₐ f
⟨⟩ᶠ A f g = ⟨⟩ˢ f A (⨁ₜ f) g

π⟨⟩ᵢ : (A : AbGrp ℓ) → (f : FinSetₐ ℓ) → (g : (x : ⟨ fst f ⟩) → A →ₐ snd f x) → (x : ⟨ fst f ⟩) →
  ∘ₐ A (⨁ₐ f) (snd f x) (⟨⟩ᶠ A f g) (πᵢ f x) ≡ g x
π⟨⟩ᵢ A f = ⟨⟩πs f A (⨁ₜ f)

∃!UPπf : (A : AbGrp ℓ) → (f : FinSetₐ ℓ) → (g : (x : ⟨ fst f ⟩) → A →ₐ snd f x) → (h : A →ₐ ⨁ₐ f) →
  ((x : ⟨ fst f ⟩) → ∘ₐ A (⨁ₐ f) (snd f x) h (πᵢ f x) ≡ g x) → h ≡ ⟨⟩ᶠ A f g
∃!UPπf A f = ∃!UPπs f A (⨁ₜ f)

∃!UPπfId : (f : FinSetₐ ℓ) → (g : ⨁ₐ f →ₐ ⨁ₐ f) →
  ((x : ⟨ fst f ⟩) → ∘ₐ (⨁ₐ f) (⨁ₐ f) (snd f x) g (πᵢ f x) ≡ πᵢ f x) → g ≡ idₐ (⨁ₐ f)
∃!UPπfId f = ∃!UPπsId f (⨁ₜ f)

⟨⟩⃖f : (A B : AbGrp ℓ) (f : FinSetₐ ℓ) → (g : A →ₐ B) → (h : (x : ⟨ fst f ⟩) → B →ₐ snd f x) →
  ∘ₐ A B (⨁ₐ f) g (⟨⟩ᶠ B f h) ≡ ⟨⟩ᶠ A f (λ x → ∘ₐ A B (snd f x) g (h x))
⟨⟩⃖f A B f = ⟨⟩⃖s f A B (⨁ₜ f)

[]ᶠ : (f : FinSetₐ ℓ) → (A : AbGrp ℓ) → ((x : ⟨ fst f ⟩) → snd f x →ₐ A) → ⨁ₐ f →ₐ A
[]ᶠ f A g = []ˢ f (⨁ₜ f) A g

κ[]ᵢ : (f : FinSetₐ ℓ) → (A : AbGrp ℓ) → (g : (x : ⟨ fst f ⟩) → snd f x →ₐ A) → (x : ⟨ fst f ⟩) →
  ∘ₐ (snd f x) (⨁ₐ f) A (κᵢ f x) ([]ᶠ f A g) ≡ g x
κ[]ᵢ f = []κs f (⨁ₜ f)

∃!UPκf : (f : FinSetₐ ℓ) (A : AbGrp ℓ) → (g : (x : ⟨ fst f ⟩) → snd f x →ₐ A) → (h : ⨁ₐ f →ₐ A) →
  ((x : ⟨ fst f ⟩) → ∘ₐ (snd f x) (⨁ₐ f) A (κᵢ f x) h ≡ g x) → h ≡ []ᶠ f A g
∃!UPκf f = ∃!UPκs f (⨁ₜ f)

∃!UPκfId : (f : FinSetₐ ℓ) (g : ⨁ₐ f →ₐ ⨁ₐ f) →
  ((x : ⟨ fst f ⟩) → ∘ₐ (snd f x) (⨁ₐ f) (⨁ₐ f) (κᵢ f x) g ≡ κᵢ f x) → g ≡ idₐ (⨁ₐ f)
∃!UPκfId f = ∃!UPκsId f (⨁ₜ f)

[]⃗f : (f : FinSetₐ ℓ) (A B : AbGrp ℓ) → (g : (x : ⟨ fst f ⟩) → snd f x →ₐ A) → (h : A →ₐ B) →
