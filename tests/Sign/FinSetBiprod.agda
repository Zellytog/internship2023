...
module Sign.FinSetBiprod where

private
  variable
    ℓ : Level

FinSetₐ : ∀ ℓ → Type (ℓ-suc ℓ)
FinSetₐ ℓ = Σ[ X ∈ FinSet ℓ ] (⟨ X ⟩ → AbGrp ℓ)

Indexᶠ : FinSetₐ ℓ → Type ℓ
Indexᶠ = ⟨_⟩ ∘ fst

δFinₛ : (f : FinSetₐ ℓ) → (a b : ⟨ fst f ⟩) → snd f a →ₐ snd f b
δFinₛ f a b = decRec (J (λ b → λ p → snd f a →ₐ snd f b) (idₐ (snd f a)))
  (λ _ → 0ₐ (snd f a) (snd f b)) (isFinSet→Discrete (f .fst .snd) a b)

FinSetPre× : ∀ {ℓ} → FinSetₐ ℓ → Type (lsuc ℓ)
FinSetPre× {ℓ = ℓ} f = Σ[ Z ∈ AbGrp ℓ ] ((x : ⟨ fst f ⟩) → Z →ₐ (snd f x))

FinSetPre+ : ∀ {ℓ} → FinSetₐ ℓ → Type (lsuc ℓ)
FinSetPre+ {ℓ = ℓ} f = Σ[ Z ∈ AbGrp ℓ ] ((x : ⟨ fst f ⟩) → (snd f x) →ₐ Z)

module _ (f : FinSetₐ ℓ) where
  ...
  u×s : FinSetPre× f → Type (lsuc ℓ)
  u×s Z = (A : AbGrp ℓ) → isEquiv (distrib×s Z A)

  u+s : FinSetPre+ f → Type (lsuc ℓ)
  u+s Z = (A : AbGrp ℓ) → isEquiv (distrib+s Z A)

  uπκs : AbGrp ℓ → Type (lsuc ℓ)
  ...
  compπκs Z p = []ᶜˢ ((Z , p .fst .fst) , p .fst .snd) Z (λ x →
    ⟨⟩ₚˢ ((Z , p .snd .fst) , p .snd .snd) (snd f x) (λ y → δFinₛ f x y))
    ≡ idₐ Z
  ...
  is⟨⟩κs : (A : Ab⨁ₛ) → (x : ⟨ fst f ⟩) → ⟨⟩ˢ (snd f x) A (λ y → δFinₛ f x y) ≡ κˢ A x
  is⟨⟩κs A x = sym ([]κs A ⌈ A ⌋ˢ (λ x → ⟨⟩ˢ (snd f x) A (λ y → δFinₛ f x y)) x) ∙
    cong (λ a → ∘ₐ (snd f x) ⌈ A ⌋ˢ ⌈ A ⌋ˢ (κˢ A x) a) (A .snd .snd) ∙
    rUnitMorph (snd f x) ⌈ A ⌋ˢ (κˢ A x)

  κᵢ∘πᵢ : (A : Ab⨁ₛ) → (x y : ⟨ fst f ⟩) →
    ∘ₐ (snd f x) ⌈ A ⌋ˢ (snd f y) (κˢ A x) (πˢ A y) ≡ δFinₛ f x y
  κᵢ∘πᵢ A x y =  cong (λ a → ∘ₐ (snd f x) ⌈ A ⌋ˢ (snd f y) a (πˢ A y)) (sym (is⟨⟩κs A x)) ∙
    ⟨⟩πs (snd f x) A (λ y → δFinₛ f x y) y
  ...
  ≡≃ₐ+×s Z A = sym (∃!UPκs Z ⌈ A ⌋ˢ (κˢ A) (⟨⟩ˢ ⌈ Z ⌋ˢ A (πˢ Z)) (λ x → 
    (cong (λ a → ∘ₐ (snd f x) ⌈ Z ⌋ˢ ⌈ A ⌋ˢ a (⟨⟩ˢ ⌈ Z ⌋ˢ A (πˢ Z)))
      ...
      []κs Z ⌈ Z ⌋ˢ (λ x → ⟨⟩ˢ (snd f x) Z (λ y → δFinₛ f x y)) x) ∙
      ⟨⟩⃖s (snd f x) ⌈ Z ⌋ˢ A (⟨⟩ˢ (snd f x) Z (δFinₛ f x)) (πˢ Z) ∙
      cong (λ a → ⟨⟩ˢ (snd f x) A a)
        (funExt (λ x₁ → (⟨⟩πs (snd f x) Z (δFinₛ f x) x₁)))
        ∙ is⟨⟩κs A x)))
